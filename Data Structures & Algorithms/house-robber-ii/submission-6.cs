...
public class Solution {
    ...
    public int Rob(int[] nums) {
        ...
        int robFirstMax = HouseRobberI_Soln_BetterVersion_MadeByAI(nums[..^1]); //[0, ^1) (start inclusive, end exclusive)

        // Rob Last house (without First):
        int robLastMax = HouseRobberI_Soln_BetterVersion_MadeByAI(nums[1..]);

        return Math.Max(robFirstMax, robLastMax);
    }
    
    public int HouseRobberI_Soln_BetterVersion_MadeByAI(Span<int> nums) {
