public class Solution {
    public List<List<int>> ThreeSum(int[] nums) {
        ...
        return attemptTwoPtr2(nums); //IMPORTANT: READ THIS ONE AND ITS EXPLANATIONS!!!
    }

    //TC: O(n^2) // O(nlog(n)+n^2) but n^2 dominates nlog(n) asympotitcally
    //SC: O(1)
    public List<List<int>> attemptTwoPtr1_NeetCodeSolnBasedAttempt(int[] nums) //Better done in attemptTwoPtr2 with more optimizations (kinda micro?)
    {
    ...
    public List<List<int>> attemptTwoPtr2(int[] nums)
    {
        ...
        for(int i=0; i<nums.Count();i++)
        {
            ...
            //ADDED IT A FEW MONTHS LATER ON MY REVIEW!!
            if(nums[i]>0) // => nums[l] > 0 && nums[r] > 0
                break;
            if(i>0&&nums[i]==nums[i-1])
