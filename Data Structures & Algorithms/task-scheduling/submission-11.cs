public class Solution {
    public int LeastInterval(char[] tasks, int n) {
        ...
        return NuAttempt2_Review(tasks, n);
    }

    // # Solution from 12-06-2026: REVIEW
    public int NuAttempt2_Review(char[] tasks, int n) {
        if(n<=0)
            return tasks.Length;
        int CooldownLength = n;
        int curCpuCycle = 0;

        // Part 1: Ger task freqs
        ...
        // Part 2: PriorityQueue tasks (HEAPIFY)
        PriorityQueue<(char task, int freq), int> pq = new(taskFreq.Select(kvp => ((kvp.Key, kvp.Value), -kvp.Value)));

        // Part 3: Simulate processing (including cooldown)
        Queue<(char task, int freq, int nextAvailableCycle)> cooldownQ = new();

        while(pq.Count > 0 || cooldownQ.Count > 0) {
            if(pq.Count == 0) { //Skip useless iterations
                curCpuCycle = cooldownQ.Peek().nextAvailableCycle;
            }

            while(cooldownQ.Count > 0 && cooldownQ.Peek().nextAvailableCycle <= curCpuCycle) {
                var (cdTask, cdFreq, _) = cooldownQ.Dequeue();
                pq.Enqueue((cdTask, cdFreq), -cdFreq);
            }

            var cur = pq.Dequeue();
            cur.freq--;
            if(cur.freq != 0) {
                var nextAvailable = curCpuCycle + (1 + CooldownLength);
                cooldownQ.Enqueue((cur.task, cur.freq, nextAvailable));
            }
            curCpuCycle++;
        }
