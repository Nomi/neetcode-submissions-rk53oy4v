...
public class Solution {
    ...
    const int NUM_LOWERCASE_ENGLISH_CHARS = 26;

    public List<List<string>> GroupAnagrams(string[] strs) {
        Dictionary<string, List<string>> groups = new ();

        for(int i=0; i < strs.Length; i++) {
            int[] freq = new int[NUM_LOWERCASE_ENGLISH_CHARS];
            foreach(var chr in strs[i]) {
                freq[chr-'a']++;
            }

            var groupKey = string.Join(',', freq);
            
            groups.TryAdd(groupKey, new());
            groups[groupKey].Add(strs[i]);
        }
