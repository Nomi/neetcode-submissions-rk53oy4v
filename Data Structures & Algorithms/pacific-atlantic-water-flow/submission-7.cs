public class Solution {
    ...
    public List<List<int>> PacificAtlantic(int[][] heights) { // ***** NuAttempt1 MISSING OPTIMIZATIONS RIGHT NOW!!!! *****
        solver = new
...
public class NuAttempt1_Bfs : IPacificAtlanticWaterflowSolver {
    ...
    //      AND IT PASSED ALL TESTS ON FIRST TRY/SUBMISSION (after fixing compile/syntax errors)
    ...
    public List<List<int>> Solve(int[][] heights) {
        // *IMPORTANT* NOTE: 
        // While I won't write it, a more time and space efficient solution exists.
        // (does NOT change complexities though). Here is what we can do:
        //
        // 1. Initialize the hashset(s) outside of the BFS function, 
        //    and add the initial wave of cells to it while we're 
        //    adding them to the queues.
        //    Then, we can simply remove the first `set.Add(cur);` call from our BFS while loop!
        //
        // 2. If in-place modifications are allowed, FIRST, collect one ocean's reachable 
        //    cells in a hashset.
        //    Then, SECOND, for the next one, do a separate BFS codeblock for it that
        //    uses in-place edits (=Seen) to mark for cycles (so in-place modifications must be allowed)
        //    and for all cells that we reach that are also in the hashset from first ocean, add it
        //    to the output list!
        ...
        HashSet<(int r, int c)> atlanticSet = GetReachableCells(heights, atlanticQ); 
        // tbf we could do it without a second set (or requiring separate intersection loop), but this approach is just.. cleaner? 
        // Also, that one would need explicit confirmation of input array mutability (because first BFS would use hashset for cycles, second would use marking!)
    ...
    HashSet<(int r, int c)> GetReachableCells(int[][] heights, Queue<(int r, int c)> oceanQ)
    {
        ...
        while(oceanQ.Count > 0)
        {
            ...
            set.Add(cur); // *IMPORTANT* Has to be here for cycle checks for the elements queued before this function is called !! :)
                          // Would be better off adding them to the set from the start when calculating filling the queue though!
                          // Then pass the initialized set here and modify it instead of return!
