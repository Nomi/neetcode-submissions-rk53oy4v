...
// Update from 14-06-26: Renamed `dst` to `dest` for clariy.
...
public class Solution {
    ...
    public Dictionary<int, List<(int time, int dest)>> GetAdjList(int[][] times) {
        Dictionary<int, List<(int time, int dest)>> adj = new();
        foreach(var time in times) {
            adj.TryAdd(time[0], new());
            adj[time[0]].Add((time: time[2], dest: time[1]));
        }
        return adj;
    }

    public int GetMinTotalTime(Dictionary<int, List<(int time, int dest)>> adj, int nodeCount, int start) {
        ...
        // the PQ contains (int cost, int node/dest) where time is Total Cost to get to that node from start
        ...
        // Also, please REMEMBER to do a minTime.ContainsKey(cur.dest) EVEN IF YOU DO OPTIONAL OPTIMIZATION FOR NEIGHBORS WITH THIS
        // WHY?? BECAUSE THESE DON'T GET ADDED UNTIL WE POP FROM PQ AND A PQ CAN HAVE MULTIPLE EDGES TO IT AT THE TIME!

        PriorityQueue<(int time, int dest), int> minHeap = new();
        ...
        while(minHeap.Count > 0 && nodeMinTime.Count < nodeCount) {
            ...
            if(nodeMinTime.ContainsKey(cur.dest)) // ABSOLUTELY NECCESSARY HERE!     [almost didn't have it until a neetcode.io test case failed with key already exists]
                    continue;

            totalTime = cur.time; //every time is greater than last
            nodeMinTime.Add(cur.dest, totalTime);
            
            if(!adj.ContainsKey(cur.dest)) //no neighbors
                continue;

            foreach(var nei in adj[cur.dest])
            {
                if(nodeMinTime.ContainsKey(nei.dest)) //WRONG: only need this here in new version. MISCONCEPTION! In fact, This one is just optional optimization!
                    continue;
                var minTimeToNei = nei.time + cur.time;
                minHeap.Enqueue((minTimeToNei, nei.dest), minTimeToNei);
            }
        }
    ...
    public int GetMinTotalTime__FirstTry_MostlyButNotFully_WithoutHints(Dictionary<int, List<(int time, int dest)>> adj, int nodeCount, int start) {
        if(!adj.ContainsKey(start))
            return NotFound;
        PriorityQueue<(int time, int dest), int> minHeap = new(adj[start].Select(tup => (tup, tup.time)));
        Dictionary<int, int> nodeMinTime = new();
        
        var totalTime = 0;
        while(minHeap.Count > 0 && nodeMinTime.Count < nodeCount) {
            var cur = minHeap.Dequeue();
            if(nodeMinTime.ContainsKey(cur.dest)) //already found a minimum! //need it here too for edges queued up in the heapify
                continue;
            totalTime = cur.time; //every time is greater than last
            nodeMinTime.Add(cur.dest, cur.time);
            ...
            foreach(var nei in adj[cur.dest])
            {
                if(nodeMinTime.ContainsKey(nei.dest))
                    continue;
                var minTimeToNei = nei.time + cur.time;
                minHeap.Enqueue((minTimeToNei, nei.dest), minTimeToNei);
            }
