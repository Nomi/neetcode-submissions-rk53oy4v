...
public class NuAttempt1 : IRottingOranges { //TC = O(M*N + M*N) = O(2*M*N) =  O(M*N) | Aux. SC = O(M*N)
    ...
    */
    ...
    public int SpreadInfectionBfs(Queue<(int r, int c)> q, int remainingClean, int[][] grid) // returns Time taken to infect all, OR `FailedToInfectAll` if cannot infect all!
    {   // My first submission (timed one) on this attempt was `SpreadInfectionBfs_Old`, but this approach is to switch the approach from having 2 counters to 1! 
        //(infectedCount and uninfectedCount to remainingCount), but this was done after that submission and I did not track time spent here.

        // Multi-Source BFS (level-order):
        int time = 0; //assuming no overflow
        while(q.Count > 0 && remainingClean != 0) //doing remainingClean != 0 is a massive optimization! (don't do it for more nodes than needed)
        {
            // So, it took a long while to fix the issue with failing test case on NeetCode automatic run. Not included in time completed!

            // *IMPORTANT*: [int this approach, it's remainingClean != 0]
            ...
            for(int i=0; i < levelLength && remainingClean != 0; i++)
            {
                ...
                foreach(var (dr,dc) in dirs)
                {
                    ...
                    q.Enqueue((nr, nc));
                    remainingClean--;    //Forgot to do this initially until a NeetCode automatic test run failed and I checked this!
                }
            }
        }

        return remainingClean == 0 ? time : FailedToInfectAll;
    }

    public int SpreadInfectionBfs_Old(Queue<(int r, int c)> q, int uninfectedCount, int[][] grid) // returns Time taken to infect all, OR `FailedToInfectAll` if cannot infect all!
    {
        ...
        int time = 0; //assuming no overflow
        while(q.Count > 0 && infectedCount != uninfectedCount) //infectedCount != uninfectedCount is a massive optimization! (don't do it for more nodes than needed)
        {
            // So, it took a long while to fix the issue with failing test case on NeetCode automatic run. Not included in time completed!
