...
public class Solution {
    public int ClimbStairs(int n) {     
        ...
        // *IMPORTANT*: number of ways at any step is just: 
        //  - waysAfter1Step +  waysAfter2Steps [for top down]
        // OR 
        // - ways2StepsAgo + ways1StepAgo [for bottom up]

        return 
            // RecursiveTopDownPostOrder(n, new());
    ...
    private int BottomUpSpaceOptimized(int n) {
        ...
        // We start from the base cases of our RecursiveTopDownPostOrder Solution!
        int prevPrev = 1;
        int prev = 2;
    ...
    private int RecursiveTopDownPostOrder(int n, Dictionary<int, int> memo) {
        // WRONG: if(n == 0) return 1; //i mean it is mathematically correct, but for this problem we care for the base cases 1 and 2!
        if(n == 1) return 1;
        if(n == 2) return 2;
        ...
        var waysAfter1Step = RecursiveTopDownPostOrder(n-1, memo);
        var waysAfter2Steps = RecursiveTopDownPostOrder(n-2, memo);
