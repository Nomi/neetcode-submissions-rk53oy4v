...
// ## Time:     
// (NOT including `islandsAndTreasure_Optimized_NonLevelOrder_BEST` which was done later at AI suggestion, without tracking time! )
//
...
public class NuAttempt1 : IIslandsAndTreasureSolver {
    // I hope google doesn't mind me using named tuple as classes. I just want to avoid clunkiness of classes when I have no way to take advantage of most IDE features anyway.

    const int Water = -1; //wall
    const int Treasure = 0;
    const int Land = 2147483647;
    readonly IReadOnlyList<(int dr, int dc)> dir = ((List<(int dr, int dc)>)[(-1,0), (1,0), (0, -1), (0,1)]).AsReadOnly(); //Up, Down, Left, Right //I overdid it this time just to check, I am not being this fancy in the interview. Old version was better (I don't even remember the syntax by heart!).
    // readonly (int dr, int dc)[] dir = [(-1,0), (1,0), (0, -1), (0,1)]; //Up, Down, Left, Right
    //^ Was missing [] on type earlier :O

    public void islandsAndTreasure(int[][] grid)
    {
        // Read my notebook notes for Mult-Source BFS (Matrix,  Shortest Path)

        // *IMPORTANT*:
        // In this problem, Non-Level Order version is BEST. Instead of using level to calculate distance, better 
        // to calculate the distance transitively: `1 + prevCellDistance` [for Treasures, thankfully they're 0 by default!!]
        //
        // I DIDN'T THINK OF THE NON-LEVEL ORDER VERSION OF THIS WHERE WE JUST USE THE VALUE FROM THE LAST CELL
        // HAD TO HAVE AI POINT IT OUT TO ME!

        // islandsAndTreasure_OriginalSubmission_LevelOrder //Original Submission, time taken was measured only for this
        islandsAndTreasure_Optimized_NonLevelOrder_BEST
        (grid);

        return;
    }

    public void islandsAndTreasure_Optimized_NonLevelOrder_BEST(int[][] grid) { // Done after submission, so not included in time taken calculations!
        ...
        for(int r=0; r < grid.Length; r++)
        {
            for(int c=0; c<grid[0].Length; c++) //non jagged!
            {
                ...
                // *IMPORTANT* According to AI, it is better to add the "Destination" first FOR better space AND time complexity!
                // TC without starting from destination would be O((M*N)^2) instead of O(M*N) we have now!! 
                // Why? Just read somewhere about it! (too tired to write more!)
            }
        }

        // 2. Do Multi Source BFS
        while(q.Count > 0) //note that here !=Land acts as our "cycle detection"!!
        {
            ...
            foreach(var (dr, dc) in dir)
            {
                ...
                // Checking: Bounds, Validity (is Land), Cycles (is Land acts as cycle detection too!)
                if(nr < 0 || nr >= grid.Length || 
                    nc < 0 || nc >= grid[0].Length || grid[nr][nc] != Land) //assuming no jagged arrays
                {
                    continue;
                }
                grid[nr][nc] = 1 + grid[curR][curC];         // *IMPORTANT* DIDN'T THINK OF THIS EARLIER! (shouldn't have used level order traversal here!)
    ...
    public void islandsAndTreasure_OriginalSubmission_LevelOrder(int[][] grid) {
        // Multi Source BFS             // (I kinda just... knew that this was to be used???)
        ...
        // 2. Do Level Order MultiSource BFS
        var dist = 1;
        while(q.Count > 0) //note that here !=Land acts as our "cycle detection"!!
        {
