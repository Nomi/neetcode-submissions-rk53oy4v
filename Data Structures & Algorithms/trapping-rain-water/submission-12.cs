    ...
    public int Trap(int[] height) {
        ...
        while(l <= r) {
            ...
            if(lMaxH <= rMaxH) {
                ...
                // Due to the line above, the line below is sort of auto-clamped to 0 when needed :D
                curWater = lMaxH - height[l]; //area at a specific column/index = height * 1 (because width = 1).
                l++;
            }
            else {
                rMaxH = Math.Max(rMaxH, height[r]);
                // Due to the line above, the line below is sort of auto-clamped to 0 when needed :D
                curWater = rMaxH - height[r]; //area at a specific column/index = height * 1 (because width = 1). 
                r--;
            }
            totalWater += curWater;
        }
