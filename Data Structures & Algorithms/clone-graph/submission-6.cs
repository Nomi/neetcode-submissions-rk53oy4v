...
public class NuAttempt3 : IGraphCloner {
    ...
    // cloneMap here is a way to get references to already clone nodes // 13-06-26 Update: (Should've named it cloneCache smh!)
    // BUT ALSO as a way to avoid cycles implicitly!
    
    // 13-06-26 Update: 
    // TC = O(V+E) because we go through each vertex exactly once and each edge exactly twice
    // SC = O(V) for cloneMap
