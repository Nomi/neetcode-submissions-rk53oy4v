...
public class NuAttempt1_TopoSort_KahnAlgo : ICourseSchedule {
    ...
    // MY TC WAS ALSO WRONG BTW!!!
    

    // ## Complexities: [CORRECT]
    // ### Time: 
    // *IMPORTANT*: 
    // In Topological Sort (Kahn's Algo.), we only go through each node at most once (through queue)
    // AND the inner for loop (for going through a nodes edges) GOES FOR A **TOTAL** OF E times (we don't revisit any edge again).
    // AND BOTH OPERATIONS ARE KINDA INDEPENDENT OF EACH OTHER (i.e. it doesn't matter which node actually has what edges, we just process each edge once)
    // So the Kahn's algorithm part is O(V+E)
    // 
    //  TC = O(E + V + (V+E))
    //  => TC = O(V+E);
    // ### Space:
    // *IMPORTANT*: 
    // adjList has at most V keys with V lists,
    // AND the combined length of the inner lists CANNOT exceed E (so at most E)
    // AND BOTH OPERATIONS ARE KINDA INDEPENDENT OF EACH OTHER (i.e. it doesn't matter which node actually has what edges, we just store each edge once)
    //
    //  SC= O([V+E] (adjList)+ V(indegrees) + V(queue)) 
    //  => SC = O(V+E)
    

    // *IMPORTANT*: DEPRECATED ## Complexities: [WRONG, well, not fully wrong BUT there are tighter bounds we should use!]
    // DEPRECATED ### Time: 
    // DEPRECATED TC = O(E + V + V*E_out_max) where E_out_max is the maximum edges going OUT of any vertex, which theoretically is (E-1)
    // DEPRECATED => TC = O(V*E);
    // DEPRECATED ### Space:
    // DEPRECATED SC= O(V*E (adjList)+ V(indegrees) + V(queue)) 
    // DEPRECATED => SC = O(V*E)
    public bool CanFinish(int numCourses, int[][] prerequisites) {
        ...
        // *IMPORTANT* It's better to use List<int>[] adjList because there's no need to worry about unset keys + more performant, 
        //STILL needs null checks tho (only helps avoid using an invalid index/key unlike in array where you need extra check). 
        //So, anyway, gotta be careful in both.
