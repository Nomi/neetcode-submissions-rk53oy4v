...
// - Inverse Ackerman function (represented here by InvAck or InverseAckerman) grows VERY SLOWLY. Therefore, for all realistic graph sizes (node counts), O(V) < 5!
...
public class NuAttempt1_UnionFind_BySize_WithPathCompression : IRedundantConnectionFinder {
    ...
    public int[] FindRedundantConnection(int[][] edges) {
        ...
        foreach(var edge in edges) { //TC=O(E*InvAck(V)) ~= O(E) E<= V^2
            if(!dsu.Union(edge[0],edge[1])) //TC = O(InvAck(V)) ~= O(1)
            {
                // lastRemoveableEdge = edge;
                return edge; //post submission optimization, so not included in TC calculation
                //*IMPORTANT* ^ I was too scared to do this but it was the correct move FOR THIS SPECIFIC PROBLEM. Here's why:

                // Since only 1 cycle exists AND we need to remove the edge appearing last in edge list: 
                // The first edge in edge list that creates the cycle, is the last edge we need to remove from our graph (according to problem constraints).
                //
                // now, IF, it was the other way around and we needed first edge in edge list we can remove to remove the cycle:
                // we'd iterate the edge list in reverse.
                //
                // For DFS, it is the other way around!
            }
        }
