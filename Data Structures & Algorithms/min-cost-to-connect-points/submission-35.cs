...
public class NuAttempt1_Prim : IMinCostToConnectPointsCalculator {
    ...
    //
    // > ALSO THIS ONLY WORKS WHEN GUARANTEED SUCH A POINT EXISTS (NonOptimal DOES work there, so just stick with that!)
    public int MinCostConnectPoints(int[][] points) { 
        // > ONLY WORKS WHEN GUARANTEED SUCH A POINT EXISTS (`NonOptimal` DOES work there, so just stick with that!)

        //Prim's Algo
        int V = points.Length;
        int E = V-1; 
        // For Undirected Graphs without Cycles [i.e. Forest], E <= V-1
        // For Undirected Connected Graphs without Cycles [i.e. Trees], E == V-1 
