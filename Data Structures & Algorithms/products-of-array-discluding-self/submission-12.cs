...
// Note: in my rush to prep for interviews coming up soon, I had `rollingProduct *= res[i+1]` instead of `...*= src...` earlier and 
//       had to have that pointed out by AI, but I still think it's not too big of a deal given I was rushing 
//       (and had already done it correctly for the prefix part, so not a conceptual misunderstanding).

internal class ArrProductExceptSelfNoDivideSolver_Retry {
    public int[] Solve(int[] nums) {
        var res = GeneratePrefixProduct(nums);
        FillSuffixProduct(nums, res);
        return res;
    }

    public int[] GeneratePrefixProduct(int[] src) {
