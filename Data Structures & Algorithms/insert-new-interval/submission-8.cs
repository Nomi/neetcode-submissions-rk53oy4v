public class Solution {
    public int[][] Insert(int[][] intervals, int[] newInterval) {
        IInsertInterval solver = new
                                    // Attempt1_OverEngineeredButOptimal_MustKnowBasicsForFollowUpsMaybe
                                    Attempt2_ActualInterviewVersion
                                ();
...
public class Attempt2_ActualInterviewVersion : IInsertInterval {
    ...
    public int[][] Insert(int[][] intervals, int[] newInterval) {
        ...
        // 1.1. Directly add Intervals ABSOLUTELY BEFORE new interval 
        for(; i < intervals.Length; i++) {
            if(intervals[i][End] >= nStart) //IMPORTANT: This MUST be `>=` and not `>`!! I had it wrong before AI pointed out error!
                break;
            res.Add(intervals[i]);
        }
        
        // 1.2. Merge any resulting OVERLAPS:
        for(; i < intervals.Length; i++) {
            if(intervals[i][Start] > nEnd)
                break;
            nStart = Math.Min(nStart, intervals[i][Start]);
            nEnd = Math.Max(nEnd, intervals[i][End]);
        }
        res.Add([nStart, nEnd]);

        // 1.1. Directly add Intervals REMAINING (ABSOLUTELY AFTER) new interval 
        for(; i < intervals.Length; i++) {
            res.Add(intervals[i]);
        }
...
public class Attempt1_OverEngineeredButOptimal_MustKnowBasicsForFollowUpsMaybe : IInsertInterval {
    const int Start = 0;
    const int End = 1;
