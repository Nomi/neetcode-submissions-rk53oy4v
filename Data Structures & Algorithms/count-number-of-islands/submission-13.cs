...
public class NuAttempt1 : ICountIslands {
    ...
    public int NumIslands(char[][] grid) {
        ...
        for(int r = 0; r < grid.Length; r++)
        {
            for(int c = 0; c < grid[r].Length; c++)
            {
                ...
                islands++;
                // MarkIslandVisitedDfs(grid, r, c); //BEST DFS VERSION!
                // BfsMarkIslandVisited(grid, r, c);
                BfsMarkIslandVisited_EarlyMarking__BEST_BFS(grid, r, c); //BEST BFS VERSION!
            }
        }
    ...
    public void BfsMarkIslandVisited_EarlyMarking__BEST_BFS(char[][] grid, int r, int c)
    {
        Queue<(int r, int c)> queue = new();
        
        // 1. Mark and Enqueue the seed immediately
        grid[r][c] = Seen; 
        queue.Enqueue((r, c));
        ...
        while (queue.Count > 0)
        {
            ...
            // 2. Use a loop for directions to reduce "copy-paste" errors
            for (int i = 0; i < 4; i++)
            {
                int nr = cur.r + dr[i];
                int nc = cur.c + dc[i];

                // 3. Boundary check + "Is it land?" check
                if (nr >= 0 && nr < grid.Length && 
                    nc >= 0 && nc < grid[nr].Length && 
                    grid[nr][nc] == Isle)
                {
                    grid[nr][nc] = Seen; // Mark EARLY      //*IMPORTANT*: Helps keep queue size down as only 1 neighbor of a cell will enqueue it!
                    queue.Enqueue((nr, nc));
                }
            }
        }
    ...
    public void BfsMarkIslandVisited_LateMarking_NotBestBfs(char[][] grid, int r, int c)
    { //Took 10 minutes to write, but couldn't come up with the better, more space efficient EarlyMarking approach! (BfsMarkIslandVisited_EarlyMarking__BEST_BFS)
        Queue<(int r, int c)> queue = new();
        queue.Enqueue((r,c));
