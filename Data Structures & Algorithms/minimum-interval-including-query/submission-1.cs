public class Solution {
    ...
    public int[] MinInterval(int[][] intervals, int[] queries) {
        return
            Attempt1__SaferToWrite_In_HighStress_And_Interviews
            // Attempt1__microOptimized
        (intervals, queries);
    }

    int[] Attempt1__SaferToWrite_In_HighStress_And_Interviews(int[][] intervals, int[] queries) {
        ...
        foreach(var queryTime in sortedQueries) {
            ...
            // Dequeue Irrelevant intervals (that were here either before enqueuing new ones above or from that batch! )
    ...
    int[] Attempt1__microOptimized(int[][] intervals, int[] queries) {
        Dictionary<int, int> queryTimeToMinInterval = new();
