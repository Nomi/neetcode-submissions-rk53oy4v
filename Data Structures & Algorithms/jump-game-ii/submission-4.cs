public class Solution {
    ...
    private int Greedy(int[] nums) {
        ...
        // 14-04-26 Update: Switching names of cur and curFathestJumpIdx here and elsewhere.
        int jumps = 0; //, cur = 0, curFarthestJumpIdx = 0;
        int windowStart = 0, windowEnd = 0; //Marks as start and end of reachable window [start,end] (start is there to track ones we could have already reached before)
        while(windowEnd < Destination) {

            int nxtFarthestJumpIdx = 0;

            for(int nxt = windowStart; nxt <= windowEnd; nxt++) {
            ...
            if(nxtFarthestJumpIdx <= windowStart) throw new Exception("Unreachable");

            windowStart = windowEnd + 1;
            windowEnd = nxtFarthestJumpIdx;
            jumps++;
        }
